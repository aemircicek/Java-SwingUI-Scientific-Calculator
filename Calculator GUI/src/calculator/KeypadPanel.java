...
public class KeypadPanel extends JPanel {
					...
					if (CalculatorLogic.isFirst) {
						...
						if (number2 == Math.floor(number2)) {
							...
							if (label.equals("÷")) {
								...
								if (displayPanel.getField().getText().contains("(")
										&& !displayPanel.getField().getText().contains(")")) {
									displayPanel.getField()
											.setText(displayPanel.getField().getText() + (int) number2 + ")" + " * ");
	...
	private void addSpecialButtons() {
		...
		KeyBindingsUtil.setupKeyBindings(displayPanel, btn0, btn1, btn2, btn3, btn4, btn5, btn6, btn7, btn8, btn9,
				btndot, btnplus, btnsubs, btnmult, btndiv, btnequals, btnprntopen, btnprntclose);
	}
}